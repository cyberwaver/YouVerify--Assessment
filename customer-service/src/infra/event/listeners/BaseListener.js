...
class BaseListener {
  ...
  listenTo(eventInfoArray, ...handlers) {
    handlers.forEach((handler) => appEvent.registerFor(eventInfoArray[0], this.handlerWrapper(handler)));
  }
  ...
  handlerWrapper(handler) {
    const innerHandlerWrapper = async (eventMeta) => {
      try {
        ...
        console.log(
          `Handler: ${this._getHandlerEventName(handler)} failed for EVENT: ${eventMeta.name} =>>> `,
          err
        );
      }
    };
