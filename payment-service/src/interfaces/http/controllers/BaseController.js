...
class BaseController {
  ...
  async execute(res) {
    let executingTask;
    const { asyncTask, asyncTaskArgs, onFailure, onSuccess } =
      this.__executionContext;
    if (typeof asyncTask === "object")
      executingTask = asyncTask.execute(...asyncTaskArgs);
    else if (typeof asyncTask === "function")
      executingTask = asyncTask(...asyncTaskArgs);
